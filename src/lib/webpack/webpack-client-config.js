...
const htmlPlugin = (config, src) => {
	const htmlWebpackConfig = ({ url, title }) => ({
		...
		favicon: exists(resolve(config.src, 'assets/favicon.ico'))
			? 'assets/favicon.ico'
		...
		ssr(params) {
			return config.prerender
				? prerender({ dest: config.dest, src }, { ...params, url })
				: '';
		}
	});
	const pages = readJson(resolve(config.cwd, config.prerenderUrls || '')) || [
		{ url: '/' }
	];
	return addPlugins(
		pages.map(page => new HtmlWebpackPlugin(htmlWebpackConfig(page))).concat([
